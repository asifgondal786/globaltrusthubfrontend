...
class TrustScoreProvider extends ChangeNotifier {
  ...
  Future<void> loadForUser({
    ...
    await calculateTrustScore(
      userId: userId,
      verificationLevel: profileData['verification_level'] ?? 0,
      documentsVerified: profileData['documents_verified'] ?? 0,
      identityConfirmed: profileData['identity_confirmed'] ?? false,
      successfulTransactions: profileData['successful_transactions'] ?? 0,
      failedTransactions: profileData['failed_transactions'] ?? 0,
      totalValue: (profileData['total_value'] ?? 0).toDouble(),
      disputeRate: (profileData['dispute_rate'] ?? 0).toDouble(),
      totalReviews: profileData['total_reviews'] ?? 0,
      averageRating: (profileData['average_rating'] ?? 0).toDouble(),
      verifiedReviews: profileData['verified_reviews'] ?? 0,
      daysActive: profileData['days_active'] ?? 0,
      loginFrequency: (profileData['login_frequency'] ?? 0).toDouble(),
      profileCompleteness: (profileData['profile_completeness'] ?? 0).toDouble(),
      responseRate: (profileData['response_rate'] ?? 0).toDouble(),
      reportedCount: profileData['reported_count'] ?? 0,
      scamFlags: profileData['scam_flags'] ?? 0,
      positiveInteractions: profileData['positive_interactions'] ?? 0,
      communityContributions: profileData['community_contributions'] ?? 0,
    );
  }
